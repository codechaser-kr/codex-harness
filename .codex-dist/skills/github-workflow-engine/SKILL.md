...
중단된 흐름을 재개할 때 사용자 요청이 직전 응답에서 제시한 선택지 중 하나를 명시적으로 선택하거나 확정된 액션 실행을 지시한 것이 아니면 Workflow Engine은 먼저 현재 워크플로우 상태를 요약하고, State Transition Rule에서 현재 조건을 만족하는 `액션` 선택지를 제시한다.
...
- `claude/code-review` 리뷰 실행 전에는 Claude CLI 인증, base/head 브랜치를 target으로 전달해 `/code-review` command를 `claude -p`로 호출할 수 있는 Claude CLI 실행 가능 상태, Codex에서 Claude를 호출하기 위한 `sendbird/cc-plugin-codex`의 `$cc:setup` 준비 상태를 확인한다.
...
`claude/code-review`는 `$cc:review` companion review가 아니라 base/head 브랜치를 target으로 전달해 Claude Code의 `/code-review` command를 호출한다. `$cc:review` companion review는 `claude/code-review`에 매핑하지 않는다. GitHub comment 게시, review thread 게시, 파일 수정은 Workflow Engine 후속 단계에서만 수행한다.
...
정책검토 이슈에서 `정책 검토 결과 확정`이 체크되어 있고 `결정된 정책을 관련 설계 문서에 반영`이 미체크이면 정책 결정 반영은 완료된 단계로 보되 설계 문서 반영은 완료되지 않은 단계로 본다. 이때 현재 액션은 open PR, 마지막으로 확정된 Human Checkpoint, 마지막 관련 액션을 함께 대입해 선택한다. 설계 반영 계획 확정 근거가 없으면 `설계 문서 반영 계획`이고, 설계 반영 브랜치나 PR이 이미 존재하면 기능 구현 흐름의 해당 중간 액션으로 재개한다. `설계 문서 반영 계획` 액션은 반영 대상, 제외 대상, 반영 단위, 검증 기준, 후속 기능변경 이슈 필요 여부를 사용자에게 제시하고, 사용자가 계획을 확정한 뒤에만 `설계 문서 반영 착수`로 전이한다.
...
6. 확정된 변경과 확정된 커밋 메시지로만 커밋한다. 사용자가 특정 커밋 메시지를 확정하기 전에는 `git commit`을 실행하지 않는다.
...
12. 선택된 리뷰 실행 모드로 PR diff와 이슈 맥락, 출력 템플릿 요구사항을 준비해 리뷰 결과를 생성한다. `claude/code-review`는 `$cc:review`가 아니라 base/head 브랜치를 target으로 전달해 Claude Code의 `/code-review` command를 호출한다.
...
19. 피드백 수정 변경이 있으면 전역 `commit` 스킬로 커밋 메시지 후보를 제안한다. `수용`은 파일 수정 승인만 의미하며 커밋 메시지 확정이 아니다. 사용자가 특정 커밋 메시지를 확정하기 전까지 `git commit`을 실행하지 않고, 확정된 변경과 확정된 커밋 메시지로만 커밋한다.
20. 피드백 수정 커밋을 원격 head branch에 push한다.
21. 피드백 수정 push 후 현재 처리 중인 피드백의 thread/comment id 또는 marker 요약 피드백 항목이 현재 Workflow Engine 실행의 `review-comment` 게시 추적 근거와 일치하는지 확인한다. 현재 처리 중인 피드백과 일치하는 추적 근거가 있는 경우에만 해당 review thread 또는 요약 피드백 항목에 `commit-hash 수정했습니다.` 형식으로 댓글을 남긴다. 판단 기준은 작성자 계정, bot 이름, 리뷰 문체가 아니라 현재 리뷰 코멘트 게시 액션의 게시 결과 또는 같은 실행 로그에 남은 추적 근거다. 추적 근거가 없거나 현재 처리 중인 피드백이 현재 실행에서 게시한 피드백인지 확정할 수 없으면 `피드백 수정 댓글` 액션을 생략하고, 외부 리뷰 도구, 사람이 남긴 피드백, 이전 실행에서 생성된 피드백에는 `commit-hash 수정했습니다.` 형식의 자동 댓글을 남기지 않는다. 별도 답글 요청이 있을 때만 외부 피드백 형식이나 사용자가 지정한 문구에 맞춘다.
22. 피드백 수정 push와 수정 댓글 게시 또는 생략 판단이 끝나면 해당 피드백을 resolve 또는 체크할지 사용자 의도를 확인한다. 사용자 결정 전에는 review thread를 resolve하거나 요약 피드백을 체크하지 않는다.
