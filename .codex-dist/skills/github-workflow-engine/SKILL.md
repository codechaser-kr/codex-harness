...
21. 피드백 수정 push 후 현재 Workflow Engine 실행이 `review-comment`로 게시한 thread/comment id 또는 marker 요약 피드백 항목 추적 근거가 있는지 확인한다. 추적 근거가 있는 경우에만 해당 review thread 또는 요약 피드백 항목에 `commit-hash 수정했습니다.` 형식으로 댓글을 남긴다. 판단 기준은 작성자 계정, bot 이름, 리뷰 문체가 아니라 현재 리뷰 코멘트 게시 액션의 게시 결과 또는 같은 실행 로그에 남은 추적 근거다. 추적 근거가 없거나 현재 실행이 게시한 피드백인지 확정할 수 없으면 `피드백 수정 댓글` 액션을 생략하고, 외부 리뷰 도구, 사람이 남긴 피드백, 이전 실행에서 생성된 피드백에는 `commit-hash 수정했습니다.` 형식의 자동 댓글을 남기지 않는다. 별도 답글 요청이 있을 때만 외부 피드백 형식이나 사용자가 지정한 문구에 맞춘다.
