...
| 항목 | 검증 모드 규칙 |
| --- | --- |
| 명시적 활성화 | registry의 strict six-field closed contract는 항상 검증한다. 현재 사용자 요청이 validation mode를 명시한 경우에만 유효한 분류 값을 diagnostic strategy로 해석하며, 활성화 전에는 ordinary workflow를 변경하지 않는다. |
...
| deterministic 경로 | parser, adapter, evaluator는 fan-out하지 않고 진단 대상 식별에 필요한 경우 고정 입력으로 한 번만 실행한다. comparator만 수집된 request/receipt에 한 번 실행하며 pass/mismatch 또는 `unanimous_outcome`을 adapter/evaluator에 다시 전달하지 않는다. |
| registry 분류 | 명시적 활성화 뒤에만 진단 대상 `registered_executor_reference`를 한 번 resolve하고 유효한 `execution_class`, `validation_strategy`를 비교 전략 선택에 사용한다. 유효한 분류 값은 activation 또는 ordinary Definition evaluation/transition 선택의 입력이 아니다. 누락·추가·잘못된 분류, 중복 ID와 legacy registry 형식은 모든 실행에서 strict registry validation 실패다. |
| strategy | `deterministic_tool/run_once`는 반복 진단 대상이 아님을 보고하고 종료한다. `llm_session/semantic_consensus`는 의미 outcome 관찰, `llm_session/isolated_patch_consensus`는 target editor의 격리 patch 관찰을 수행한다. 다른 조합은 중단한다. |
...
| isolated patch 예정 session set | request의 `planned_session_relation`은 `ten_independent_isolated_execution_sessions`이고 `planned_session_slots`는 index 1..10의 닫힌 entry 정확히 10개다. 각 entry는 known ID 또는 `pending_tool_issued`인 planned execution session/workspace ID를 가지며 known ID는 고유하다. |
| read-only plan | semantic session은 primary·GitHub·comment·branch·commit·PR을 변경하지 않고 전체 normalized outcome 또는 외부 변경의 read-only plan만 반환한다. 파일 편집은 동일 baseline의 격리 workspace 안에서만 허용한다. |
| 완료 대기와 비교 | 유한 wait-all로 10개를 모두 수집한 뒤 stdin envelope `{ request, receipts }`를 comparator에 한 번 전달한다. comparator는 정규화된 전체 outcome의 전원 일치만 pass하고 `unanimous_outcome`을 반환한다. |
| 중단 | mismatch, timeout, blocked, 환경 불일치, duplicate/missing session/workspace ID, 외부 부작용이면 상태 변경 전 중단한다. 실행 중 session은 종료·close하며 retry, majority, representative adoption은 금지한다. |
| 진단 종료 | pass와 mismatch를 사용자 판단용 diagnostic observation으로 반환한다. `unanimous_outcome`도 원 결과로 채택하지 않으며 validation session/workspace는 primary 또는 외부 상태를 변경하지 않는다. 결과로 Workflow Definition transition을 선택·완료·진행하거나 normal workflow를 자동 재개하지 않는다. |
...
| 증거 경계 | Node scripts는 agent를 호출하지 않고 request/receipt 검증·정규화·비교만 한다. mock/control-plane fixture는 live evidence가 아니다. `.harness/logs/github-workflow-log.md`는 consensus 입력·출력으로 읽거나 쓰지 않는다. |

검증 모드의 사용자 반환에는 `request_id`, snapshot source, 10개 session ID, 필요 시 10개
workspace ID와 comparator 결과를 포함한다. isolated baseline은 validation request/receipt에서, isolated patch
data는 comparator의 `unanimous_outcome`에서 읽고 top-level proof field로 복제하지 않는다. 실제 session
fan-out은 Workflow Engine/target editor가 수행하고 comparator는 agent 실행을 수행하지 않는다.
unanimous 결과도 진단 관측이며 live 수용이나 다음 전이 근거가 아니다.
...
| 예정 세션 관계 식별 가능 | `orchestration_session_id`와 `planned_session_relation`이 있다. `planned_session_relation`이 `same_session`이면 `planned_execution_session_id`가 `orchestration_session_id`와 같거나 명시적 `same_session` 표시가 있다. `separate_execution_session`이면 알려진 `planned_execution_session_id` 또는 도구 발급 대기 `pending_tool_issued`와 근거가 있다. 검증 모드의 isolated patch이면 `planned_session_relation = ten_independent_isolated_execution_sessions`, `planned_execution_session_id = not_applicable`, `planned_execution_session_id_not_applicable_reason = validation_consensus_uses_session_set`이고, authoritative `planned_session_slots`가 `session_index`, `planned_execution_session_id`, `planned_workspace_id`만 가진 닫힌 entry 정확히 10개인지 확인한다. slot은 index 1..10 순서이며 두 planned ID는 known 고유 ID 또는 `pending_tool_issued`다. |
...
| 검증 session set 결과 식별 가능 | 예정 관계가 `ten_independent_isolated_execution_sessions`이면 `consensus_session_ids`와 `consensus_workspace_ids`가 actual ID의 authoritative 필드다. 완전한 진단 또는 10개 fan-out 뒤 중단 결과는 두 배열에 request slot과 대조된 고유 ID를 정확히 10개 기록하고 `actual_execution_session_id = not_applicable`, `actual_execution_session_id_not_applicable_reason = validation_consensus_uses_session_set`, `actual_session_relation = ten_independent_isolated_execution_sessions`를 기록한다. known planned ID는 같은 index의 actual ID와 같아야 하며 `pending_tool_issued`는 실제 발급 ID로 해소한다. 이 사유는 fan-out 전 `execution_session_not_started`와 다르다. 일부만 시작된 session set은 발급된 ID와 실패 근거를 보존하되 이 판정을 통과하지 못하고 validation 중단으로 처리한다. 일반 모드에는 이 조건을 적용하지 않는다. |
...
`planned_session_relation = ten_independent_isolated_execution_sessions`에는 위 표의 단일
`same_session`/`separate_execution_session` ID 비교를 적용하지 않는다. 이 경우 요청-결과 상관관계는
`request_id`, orchestration session, 10개 planned slot과 authoritative actual session/workspace 배열의
index별 대조로 수행한다. `actual_execution_session_id`의
`validation_consensus_uses_session_set`은 실제 session set이 존재한다는 뜻이고,
`execution_session_not_started`는 session set이 시작되지 않았다는 뜻이므로 서로 대체할 수 없다.
...
| Target Harness Code Editor 출력 사용 가능 | ordinary 실행에서 target editor가 `Target Harness Code Editor 선택 가능`을 통과한 입력으로 실제 호출된 경우에만 기존 `구조화 실행 결과 사용 가능`과 `실행 범위 준수`를 적용한다. 일반 모드의 성공 또는 실제 execution session ID가 발급된 뒤의 중단이면 반환된 `routing_status`, `selected_role_id`, `agent_config_path`, `local_skill_path`, `routing_evidence`, `model`, `model_reasoning_effort`, `sandbox_mode`가 Workflow Engine이 검증해 전달한 입력 라우팅 결과와 모두 일치하며 실제 실행 주체가 선택 역할의 agent config, local skill, model, reasoning, sandbox를 사용한 별도 execution session임이 확인된다. validation mode 결과는 같은 라우팅 대조와 `검증 session set 결과 식별 가능`을 확인하고, isolated baseline은 request/receipt에서, manifest/canonical patch/digest는 comparator outcome에서 읽어 사용자에게 진단으로만 제시한다. 이 결과를 출력 사용 가능 판정, 구조화 실행 성공 또는 다음 transition에 연결하지 않는다. 실행 세션 미시작 중단이면 `routing_status`만 `aborted`로 전이할 수 있고 나머지 라우팅 고유 필드는 검증해 전달한 입력과 항상 일치해야 하며, 전용 중단 조건과 세션 `not_applicable` 사유 대조를 통과해야 한다. 불완전 fan-out, 선택 전 중단, 구조화 요청 식별 불능, 검증된 라우팅 입력 누락은 target editor 출력 사용 가능 판정 대상이 아니다. |
