...
9. 각 역할의 agent 파일명, skill 디렉터리명, 입력/출력, 다음 역할, sandbox 정책, description 초안뿐 아니라 대표 시작 경로, 우선 입력 문서, 요청 유형별 하위 분기, 작업 시작 체크리스트, 판단 기준, 금지 판단, 출력 규칙, 산출 형식 기준, 학습 후보 기록 규칙, 재진입 트리거, 종료 판정 기준, 완료 기준까지 team-spec으로 정리한다.
...
5. 출력 형식에는 `학습 후보`, `승격 대상`, `생성기 환류 후보` 항목을 포함하되, 관찰이 없으면 `없음`으로 짧게 남기도록 한다.
