...
14. 역할 목적, 책임, primary scope에 MCP, 외부 Tool 호출, 토큰/키 관리, 감사 로그, AuditLog, AI 사용량/쿼터, LLM Provider, 프롬프트/응답 로그 제외, 민감정보, 보안 정책 같은 정책 도메인이 있으면 관련 AI/MCP/보안/감사 정책 문서를 우선 입력에 포함한다.
15. 관련 정책 문서는 고정 파일명이나 경로 패턴으로 가정하지 말고, 저장소의 문서 제목, 본문 키워드, 목차, 앱별 `AGENTS.md` 참조를 읽어 의미상 AI/MCP/보안/감사/개인정보 정책 원천에 해당하는 후보를 탐색한다.
16. 정책 도메인을 맡는 역할이 있는데 관련 정책 문서가 없으면, team-spec이나 로그에 대체 원천과 보류 판단을 남긴다.
17. 설계/정책/사양 문서는 team-spec 상단의 `설계 원천 우선순위` 또는 `설계 원천 인벤토리`에 포함하고, 운영 로그와 세션 요약 같은 재진입 문서는 설계 원천 우선순위에 넣지 않는다.
18. team-spec 상단에는 왜 그 역할명이 현재 저장소의 도메인과 실패 경계를 더 잘 설명하는지 도메인 근거를 함께 남긴다.
19. 새 역할명을 만들 때는 저장소 용어와 실패 경계를 드러내는 이름을 우선하고, `role_id`는 snake_case, 표시 이름과 파일명은 kebab-case로 정리한다.
20. `analyst`, `architect`, `orchestrator`, `validator` 같은 추상 직무명만 단독으로 역할명에 쓰지 않고, 반드시 저장소 고유 용어나 책임 축을 붙인다.
21. `## 최종 역할 인벤토리`는 반드시 fenced `text` 블록으로 남긴다. 헤더와 모든 역할 행은 같은 블록 안에 있어야 하며, inline code 행을 여러 줄 나열한 결과는 Phase 2 미완료로 본다.
...
9. 역할 목적과 책임에 정책 도메인이 포함될 때 관련 정책 문서가 우선 입력에 포함됐는지 확인한다.
10. 실행 로그와 최신 세션 요약이 다음 실행의 입력으로 읽힐 수 있는지 확인한다.
11. 부족한 축이 `team-spec` 역할 정의 문제인지, agent/skill 포인터 문제인지, 오케스트레이션 문제인지 분류한다.
