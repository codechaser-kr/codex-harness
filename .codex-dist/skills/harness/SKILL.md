...
- `초기` 탐색 상태에서는 질문과 탐색 재확인을 우선하고, `제한적` 상태에서는 필요한 문서만 선택 갱신하며, `충분` 상태에서는 더 구체적인 구조와 검증을 적용한다.
...
- `scripts/harness-verify.sh`가 실패하면 완료로 간주하지 말고, 누락된 구조를 먼저 다시 정리한다.
...
- 리포트와 역할 설계는 탐색 결과가 수집된 뒤에만 결과형 문서로 정리한다.
...
3. 현황에 따라 실행 모드를 나눈다.
4. 기존 하네스가 있으면 덮어쓰기보다 필요한 문서를 다시 쓰는 쪽을 우선하고, drift 가능성을 확인한다.
5. 감사 결과는 이후 init/update/verify의 입력으로 계속 사용한다.

### 실행 모드

- `신규 구축`: 로컬 역할 스킬, 보고서, 로그 구조가 거의 없는 상태. `harness-init.sh` 중심으로 시작한다.
- `기존 확장`: 일부 역할 스킬, 보고서, 로그 구조가 이미 있는 상태. `harness-update.sh`로 필요한 부분만 다시 정리한다.
