...
4. 저장소 근거가 적은 경우에도 사용자 요청에 담긴 도메인 설명을 Phase 1의 정상 입력으로 삼는다. 이 경우 `project-setup.md`에는 최소한 도메인 또는 만들 제품, 목표 사용자나 운영 주체, 첫 성공 시나리오, 품질 실패 비용이 큰 지점, 아직 코드가 없어 보류한 구현 경계, 코드나 설정이 생긴 뒤 재진입할 Phase를 기록하고, 파일 근거 부족은 보류 판단으로 남긴다.
...
9. 각 역할의 agent 파일명, skill 디렉터리명, 입력/출력, 다음 역할, sandbox 정책, description 초안뿐 아니라 대표 시작 경로, 우선 입력 문서, 요청 유형별 하위 분기, 작업 시작 체크리스트, 판단 기준, 금지 판단, 출력 규칙, 산출 형식 기준, 학습 후보 기록 규칙, 승격 대상 기준, 생성기 환류 후보 기준, 재진입 트리거, 종료 판정 기준, 완료 기준까지 team-spec으로 정리한다.
10. 역할의 목적, 책임, 주요 출력에서 필요한 판단 근거를 역산해 우선 입력 문서를 정한다.
11. 선행 정책, 구현 순서, 라우팅 판단, 보류 질문처럼 다음 실행을 좌우하는 출력을 맡는 역할은 그 판단에 필요한 정책/설계/로드맵 원천을 우선 입력에 포함한다.
12. 이 정합성 검사는 구현 역할뿐 아니라 계획 역할, 조율 역할, 시작 진입 역할, 운영 감사 역할에도 동일하게 적용한다.
13. 역할별 `우선 입력`에 등장하는 문서를 제품 정책, 기술 설계, 데이터 모델, AI/MCP/보안 정책, 프론트엔드/화면 사양, 로드맵, 앱별 구현 컨벤션, 운영 로그/임시 산출물로 분류한다.
14. QA 역할은 변경 diff, 대상 앱 또는 패키지의 `AGENTS.md`, team-spec의 설계 원천 우선순위 또는 설계 원천 인벤토리, 변경 범위에 해당하는 설계/정책/계약 문서, 최신 세션 요약 또는 재진입 상태를 우선 입력으로 갖는다.
15. QA 역할은 고정 문서 몇 개만 읽는 역할로 만들지 말고, 변경 범위가 제품 정책, API/계약, 데이터 모델, 보안/감사/비용/외부 연동, 화면 사양, 로드맵 상태 중 어디에 닿는지 보고 필요한 원천을 선택해 읽도록 정의한다.
16. QA 역할이 변경 범위에 필요한 원천 문서를 찾지 못하면 통과로 판정하지 않고 보류 판단, 미확인 위험, 다음 재진입 역할을 남기도록 한다.
17. 프론트엔드 역할이 상태 기반 화면, 권한 기반 화면, 도메인 엔티티 목록/상세/수정 화면, enum/API 응답 필드 기반 UI, 필터/탭/뱃지/상태 라벨을 맡으면 관련 데이터 모델 또는 API schema 원천을 우선 입력에 포함한다.
18. 프론트엔드 데이터 원천은 고정 파일명으로 가정하지 말고, 저장소 재독해 결과에서 데이터 모델 문서, API schema, OpenAPI/GraphQL schema, shared type package, backend DTO/response type, 앱별 `AGENTS.md`가 지정한 API 타입 원천 중 실제 원천을 찾는다.
19. 프론트엔드 역할은 자체 enum/status 문자열을 새로 만들지 않고 shared type이나 API response type을 우선 사용하도록 정의한다.
20. 프론트엔드 화면 상태에 필요한 데이터 원천이 없으면 API/backend 역할 재진입 조건 또는 보류 판단을 남기도록 한다.
21. 역할 목적, 책임, primary scope에 MCP, 외부 Tool 호출, 토큰/키 관리, 감사 로그, AuditLog, AI 사용량/쿼터, LLM Provider, 프롬프트/응답 로그 제외, 민감정보, 보안 정책 같은 정책 도메인이 있으면 관련 AI/MCP/보안/감사 정책 문서를 우선 입력에 포함한다.
22. 관련 정책 문서는 고정 파일명이나 경로 패턴으로 가정하지 말고, 저장소의 문서 제목, 본문 키워드, 목차, 앱별 `AGENTS.md` 참조를 읽어 의미상 AI/MCP/보안/감사/개인정보 정책 원천에 해당하는 후보를 탐색한다.
23. 정책 도메인을 맡는 역할이 있는데 관련 정책 문서가 없으면, team-spec이나 로그에 대체 원천과 보류 판단을 남긴다.
24. 설계/정책/사양 문서는 team-spec 상단의 `설계 원천 우선순위` 또는 `설계 원천 인벤토리`에 포함하고, 운영 로그와 세션 요약 같은 재진입 문서는 설계 원천 우선순위에 넣지 않는다.
25. team-spec 상단에는 왜 그 역할명이 현재 저장소의 도메인과 실패 경계를 더 잘 설명하는지 도메인 근거를 함께 남긴다.
26. 새 역할명을 만들 때는 저장소 용어와 실패 경계를 드러내는 이름을 우선하고, `role_id`는 snake_case, 표시 이름과 파일명은 kebab-case로 정리한다.
27. `analyst`, `architect`, `orchestrator`, `validator` 같은 추상 직무명만 단독으로 역할명에 쓰지 않고, 반드시 저장소 고유 용어나 책임 축을 붙인다.
28. `## 최종 역할 인벤토리`는 반드시 fenced `text` 블록으로 남긴다. 헤더와 모든 역할 행은 같은 블록 안에 있어야 하며, inline code 행을 여러 줄 나열한 결과는 Phase 2 미완료로 본다.
...
- 다음 단계 조건: 시작 진입 역할이 현재 상태를 읽고 다음 역할 또는 재진입 Phase를 일관되게 안내할 수 있음
...
5. 모든 역할 스킬이 `team-spec`의 해당 역할 섹션과 공통 출력 블록을 명확히 참조하는지 확인한다. 참조가 없거나 역할 세부 기준을 복제했다면 `운영 가능`으로 판정하지 않고 `Phase 4` 재작성 대상으로 분류한다.
6. 역할별 `우선 입력`의 설계/정책/사양 문서가 `team-spec` 상단의 `설계 원천 우선순위` 또는 `설계 원천 인벤토리`에 빠짐없이 포함됐는지 확인한다.
7. 역할 목적, 책임, 주요 출력과 우선 입력 문서가 정합적인지 확인한다.
8. 계획/조율 역할이 선행 정책, 구현 순서, 라우팅 판단, 보류 질문을 출력한다면 그 판단에 필요한 원천 문서를 우선 입력으로 갖는지 확인한다.
9. QA 역할이 변경 diff, 대상 앱 또는 패키지의 `AGENTS.md`, team-spec 설계 원천 목록, 변경 범위에 해당하는 설계/정책/계약 문서, 최신 재진입 상태를 우선 입력으로 갖는지 확인한다.
10. QA 역할이 변경 범위별 원천 선택 기준과 원천 부재 시 보류/재진입 판단을 갖는지 확인한다.
11. 프론트엔드 역할이 상태/권한/엔티티/API 필드 기반 UI를 맡을 때 데이터 모델 또는 API schema 원천을 우선 입력으로 갖는지 확인한다.
12. 프론트엔드 역할이 shared type이나 API response type을 우선 사용하고, 데이터 원천 부재 시 API/backend 재진입 또는 보류 판단을 남기도록 정의됐는지 확인한다.
13. 역할 목적과 책임에 정책 도메인이 포함될 때 관련 정책 문서가 우선 입력에 포함됐는지 확인한다.
14. 실행 로그와 최신 세션 요약이 다음 실행의 입력으로 읽힐 수 있는지 확인한다.
15. 부족한 축이 `team-spec` 역할 정의 문제인지, agent/skill 포인터 문제인지, 오케스트레이션 문제인지 분류한다.
...
- `기존 확장`과 `운영 유지보수`는 별도 갱신 명령이 아니라 `references/reentry-rules.md`를 따라 필요한 Phase부터 다시 들어가는 방식으로 다룬다.
...
이 상태는 역할 결정, 갱신 범위, 검증 강도를 맞추는 공통 제어 입력이다. 저장소 근거가 적더라도 사용자 도메인 설명이 있으면 `제한적` 입력으로 보고 Phase 1을 진행한다.

## 운영/유지보수 워크플로우

- 새 프로젝트 하네스 구성: `Phase 0` 감사 → 시작 입력 정리 → 역할 팀 설계 → 에이전트/스킬 생성 → 오케스트레이션 연결 → 운영 감사 역할 검토
- 기존 프로젝트 확장: 하네스 현황 감사 → 필요한 Phase 선택 → 영향받는 에이전트/스킬/오케스트레이션 갱신 → 운영 감사 역할 검토
- 기존 프로젝트의 구조 누락 정리: 하네스 현황 감사 → 필요한 경우 명시적 재구성 또는 초기 Phase 재진입 → 역할 자산과 오케스트레이션 정렬 → 운영 감사 역할 검토
- 기존 프로젝트의 운영 유지보수/감사: 하네스 현황 감사 → 필요한 Phase 재진입 → 역할 자산, 운영 문서, 로그 정렬 → 운영 감사 역할 검토
