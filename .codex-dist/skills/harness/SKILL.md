...
실행 모드와 상태 모드는 별개다. 예를 들어 `기존 확장` 상태라도 `에이전트 팀` 모드로 갈 수 있고, `운영 유지보수` 상태라도 `단일 역할` 모드로 시작할 수 있다. 탐색 상태도 별개다. 예를 들어 `기존 확장` 상태라도 탐색 상태가 `초기`일 수 있고, 이 경우에는 세부 역할을 늘리기보다 질문과 탐색 재확인부터 다시 시작한다.
