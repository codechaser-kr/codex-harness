...
9. 과도기 단계에서는 seed 역할의 상세 SKILL을 유지하되, team-spec에 새 역할이 들어오면 그 역할의 `.codex/skills/*` 기본 스킬도 함께 생성한다.
...
이 하네스의 목표는 고정 역할 세트를 복사하는 것이 아니라,  
...
`domain-analyst`, `harness-architect`, `qa-designer`, `orchestrator`, `validator`, `run-harness`는 현재 생성기의 과도기적 기본 집합일 뿐, 최종적으로는 `Phase 2`의 team-spec이 무엇을 정의하느냐가 더 중요하다.
