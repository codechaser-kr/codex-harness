...
3. 하네스 Phase 0 감사 결과를 기준으로 기존 로컬 하네스 구조와 충돌을 피한다.
...
9. 각 역할의 agent 파일명, skill 디렉터리명, 입력/출력, 다음 역할, reasoning 기본값, sandbox 정책, description 초안뿐 아니라 대표 시작 경로, 우선 입력 문서, 요청 유형별 하위 분기, 작업 시작 체크리스트, 판단 기준, 금지 판단, 출력 규칙, 산출 형식 기준, 학습 후보 기록 규칙, 승격 대상 기준, 생성기 환류 후보 기준, 재진입 트리거, 종료 판정 기준, 완료 기준까지 team-spec으로 정리한다.
...
- 산출: 피드백 반영 변경점, 운영 가능성 판정, 다음 하네스 재진입 Phase 제안, 학습 후보와 승격 대상
...
- 한국어만으로 의미가 모호한 기술 용어는 영어 용어를 괄호 안에 병기한다. 예를 들어 audit 의미의 `감사`는 `감사(audit)` 또는 `감사(audit)를 수행한다`로 쓰고, 감사 인사로 오해될 수 있는 종결 표현으로 쓰지 않는다.
- 생성되는 타겟 문서와 로그도 같은 용어 원칙을 따른다. 하네스 생명주기 단계는 `하네스 Phase N`, 제품/로드맵/개발 단계는 `제품 로드맵 Phase N`, `블로그 개발 Phase N`처럼 의미 영역을 붙여 쓴다.
