...
4. `AGENTS.md`에는 하네스 포인터와 진입 규칙만 간결하게 남기고, 상세 역할 규칙은 `team-spec`, `.codex/agents/*`, `.codex/skills/*`가 맡는다. `.harness/docs/*`는 판단 근거와 운영 기준을 보조한다.
...
5. 출력 형식에는 공통 학습 출력 블록을 반드시 포함한다. `새로 확인한 저장소 사실`, `반복될 수 있는 판단`, `하네스 갱신 후보`, `승격 대상`, `생성기 환류 후보`가 모두 있어야 하며, 관찰이 없으면 `없음`으로 짧게 남기도록 한다.
...
5. 모든 역할 스킬의 출력 형식에 공통 학습 출력 블록이 있는지 확인한다. 하나라도 빠졌다면 `운영 가능`으로 판정하지 않고 `Phase 4` 재작성 대상으로 분류한다.
6. 실행 로그와 최신 세션 요약이 다음 실행의 입력으로 읽힐 수 있는지 확인한다.
7. 부족한 축이 역할 정의 문제인지, 스킬 절차 문제인지, 오케스트레이션 문제인지 분류한다.
