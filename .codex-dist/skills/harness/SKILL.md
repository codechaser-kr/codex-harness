...
- 다음 단계 조건: 역할 경계, handoff 기준, 패턴 선택 이유, 동적 생성용 역할 스펙이 메타시스템 문서와 함께 고정됨
...
9. 각 역할의 agent 파일명, skill 디렉토리명, 입력/출력, handoff, sandbox 정책, description 초안뿐 아니라 대표 시작 경로, 우선 입력 문서, 요청 유형별 하위 분기, 작업 시작 체크리스트, 판단 기준, 금지 판단, 출력 계약, 산출 형식 템플릿, 재진입 트리거, 종료 판정 기준, 완료 기준까지 team-spec으로 정리한다.
...
10. 생성된 역할 스킬은 team-spec의 실행 계약 필드를 바탕으로, 대표 시작 경로, 요청 분기, 우선 입력 문서, 시작 체크리스트, 판단 기준, 출력 형식, 재진입/종료 규칙, handoff, 완료 기준이 역할마다 다르게 읽히도록 구체화한다.
...
5. 흐름 조율 담당 역할이 시작 분기, 재진입 루프, 세션 운영 규칙을 메타시스템 문서로 다시 쓴다.
...
6. 운영 감사 역할이 문서 검사자에 머무르지 않고 운영 계약 감사자로 읽히는지 확인한다.
...
2. `without-skill` 기준선과 비교해 시작 역할 판단, 질문 절제, handoff, 저장소 근거 연결, 검증 가능성이 나아졌는지 정리한다.
3. meta-system 성숙도 축에서 부족한 부분이 입력 문서 품질 문제인지, 역할 계약 문제인지, 운영 계약 문제인지 분리한다.
4. 필요한 경우 다음 재진입 시작점을 `Phase 1`~`Phase 6` 중 어디로 둘지 다시 제안한다.

이 단계는 부록이 아니라 메타시스템이 generic scaffold로 되돌아가지 않게 유지하는 운영 평가 단계다.
