...
13. 역할별 `우선 입력`에 등장하는 문서를 제품/운영 정책, 기술/계약 설계, 구조화된 데이터 원천, 외부 연동/보안 정책, 표현 계층 사양, 로드맵, 영역별 구현 컨벤션, 운영 로그/임시 산출물로 분류한다.
...
15. QA 역할은 고정 문서 몇 개만 읽는 역할로 만들지 말고, 변경 범위가 제품/운영 정책, 인터페이스 계약, 구조화된 데이터, 보안/추적성/비용/외부 연동, 표현 계층 사양, 로드맵 상태 중 어디에 닿는지 보고 필요한 원천을 선택해 읽도록 정의한다.
16. QA 역할이 변경 범위에 필요한 원천 문서를 찾지 못하면 통과로 판정하지 않고 보류 판단, 미확인 위험, 다음 재진입 역할을 남기도록 한다.
17. 표현 계층 역할이 상태, 권한, 도메인 객체, 계약 필드, 분류값, 표시 라벨처럼 구조화된 데이터에 의존하는 출력을 맡으면 관련 데이터 모델 또는 인터페이스 계약 원천을 우선 입력에 포함한다.
18. 표현 계층의 데이터 원천은 고정 파일명이나 기술명으로 가정하지 말고, 저장소 재독해 결과에서 문서화된 데이터 모델, 인터페이스 계약, 공유 타입 원천, 생산자 영역의 응답/전달 타입, 영역별 `AGENTS.md`가 지정한 타입 원천 중 실제 원천을 찾는다.
19. 표현 계층 역할은 구조화된 상태값이나 필드명을 임의로 새로 만들지 않고 저장소의 기존 타입/계약 원천을 우선 사용하도록 정의한다.
20. 표현 계층 출력에 필요한 데이터 원천이 없으면 해당 생산자 역할 재진입 조건 또는 보류 판단을 남기도록 한다.
21. 역할 목적, 책임, primary scope에 외부 시스템 연동, credential/secret, 추적성 로그, 비용/사용량 제한, 모델/서비스 제공자, 민감정보, 보안 정책 같은 정책 도메인이 있으면 관련 정책 문서를 우선 입력에 포함한다.
22. 관련 정책 문서는 고정 파일명이나 경로 패턴으로 가정하지 말고, 저장소의 문서 제목, 본문 키워드, 목차, 영역별 `AGENTS.md` 참조를 읽어 의미상 해당 정책 원천에 해당하는 후보를 탐색한다.
23. 정책성 책임을 맡는 역할이 있는데 관련 정책 문서가 없으면, team-spec이나 로그에 대체 원천과 보류 판단을 남긴다.
...
11. 표현 계층 역할이 상태/권한/도메인 객체/계약 필드 기반 출력을 맡을 때 데이터 모델 또는 인터페이스 계약 원천을 우선 입력으로 갖는지 확인한다.
12. 표현 계층 역할이 저장소의 기존 타입/계약 원천을 우선 사용하고, 데이터 원천 부재 시 생산자 역할 재진입 또는 보류 판단을 남기도록 정의됐는지 확인한다.
13. 역할 목적과 책임에 정책성 판단이 포함될 때 관련 정책 문서가 우선 입력에 포함됐는지 확인한다.
