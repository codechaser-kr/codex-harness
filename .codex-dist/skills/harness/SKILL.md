...
14. 세션 로그 기록과 종료 요약 갱신은 선택 사항이 아니라 하네스 실행의 필수 완료 조건이다.
...
- 하네스 실행을 시작하면 즉시 `bash scripts/harness-log.sh --new-session ...`으로 세션 시작 이벤트를 남긴다.
- 세션 시작 로그에는 최소한 `timestamp`, `session id`, `user request`, `entry role`, `planned roles`, `expected outputs`를 남긴다.
- 각 역할 또는 subagent 완료 뒤에는 `bash scripts/harness-log.sh`로 `role name`, `result status`, `input summary`, `output summary`, `changed files`, `unresolved risks`를 누적한다.
...
- 최종 응답 전에는 반드시 `bash scripts/harness-session-close.sh`로 `.harness/logs/latest-session-summary.md`를 갱신한다.
...
6. description 초안은 실제 요청에서 트리거될 수 있도록 구체적으로 작성한다.
...
10. 마지막 종료 세션이 `session-events.tsv`, `session-log.md`, `latest-session-summary.md`에 모두 반영됐는지 확인한다.
...
6. 마지막 실행 세션이 `.harness/logs/session-events.tsv`, `.harness/logs/session-log.md`, `.harness/logs/latest-session-summary.md`에 같은 세션 ID로 남아 있다.
