...
9. 각 역할의 agent 파일명, skill 디렉토리명, 입력/출력, handoff, sandbox 정책, description 초안뿐 아니라 우선 입력 문서, 작업 시작 체크리스트, 판단 기준, 금지 판단, 출력 계약, 완료 기준까지 team-spec으로 정리한다.
...
10. 생성된 역할 스킬은 team-spec의 실행 계약 필드를 바탕으로, 우선 입력 문서, 시작 체크리스트, 판단 기준, 출력 계약, handoff, 완료 기준이 역할마다 다르게 읽히도록 구체화한다.
