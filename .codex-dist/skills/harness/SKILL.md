...
저장소를 탐색해 근거를 수집하고, 그 결과를 공통 입력으로 삼아 역할 팀, 운영 구조, QA/검증 흐름, 시작 진입 역할(`run-harness`), 상태 점검 / 정렬 / 개선 루프를 설계한다.
...
2. Codex에서는 주 에이전트가 흐름을 잡고, 독립 입력과 독립 산출이 있는 좁은 작업만 서브에이전트로 병렬 위임한다.
3. 이 스킬은 한 번 생성하고 끝나는 도구가 아니다. 실행할 때마다 피드백을 반영해 에이전트 정의, 스킬, `AGENTS.md`를 현재 상태에 맞게 갱신한다.
...
이 단계의 핵심은 문서를 늘리는 데 있지 않다. **운영 가능한 역할 팀**을 설계하는 데 있다.
...
2. 각 역할의 작업 절차, 입력/출력, 책임 범위는 SKILL.md 하나에 모은다.
...
이 단계의 목적은 스크립트 실행이 아니다. 역할 스킬이 각 프로젝트의 구조와 운영 맥락에 맞는 입력 문서와 메타시스템 문서를 직접 작성하게 하는 것이다.
...
5. 생성기 자체 준비도는 `generator-readiness-checklist.md`로 점검하고, 타겟 하네스 충분성은 실제 생성 결과가 있는 타겟 프로젝트에서만 평가한다.
...
- 생성 문서와 예시에는 사용자 홈 디렉토리나 절대경로를 하드코딩하지 않는다. 실행 예시는 상대경로나 저장소 기준 경로를 쓴다.
...
- `제한적`: 사용자 입력이 있어 방향을 좁힐 수 있지만, 최종 판단은 역할 스킬이 저장소를 다시 읽은 뒤 내려야 하는 상태.

이 상태는 역할 결정, update 범위, verify 강도를 맞추는 공통 제어 입력이다.
...
init는 자동 판단 보류 메모만 만든다. 나머지 문서는 역할 스킬이 직접 작성한다.
