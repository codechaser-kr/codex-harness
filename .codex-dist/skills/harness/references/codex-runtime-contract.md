...
- 셸 스크립트는 설치, 호환, 정적 검증을 돕는 수단이다. 생성 판단의 단일 진실원천으로 쓰지 않는다.
...
5. 셸 스크립트는 위 계약을 보조하거나 검증할 수 있다. 단, 계약보다 앞서 생성 판단을 확정하지 않는다.
...
- 스크립트 없이도 Codex가 문서와 스킬 계약만 읽고 생성 절차를 수행할 수 있다.
- 스크립트가 남아 있더라도 설치, 호환, 정적 검증 보조 수단으로만 설명된다.
