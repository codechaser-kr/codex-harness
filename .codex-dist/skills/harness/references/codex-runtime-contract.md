...
첫 `spawn` 전에 주 에이전트는 현재 host에 callable `close_agent`가 있는지 확인한다. 없으면 subagent ID를
발급하지 않고 동일한 역할 계약을 주 에이전트가 순차 실행한다. Harness의 정상 실행은 보조 위임 자체를
필수 완료 조건으로 두지 않으며 `interrupt_agent`, timeout 또는 host 자동 정리를 close로 간주하지 않는다.
...
작업 경계가 위임에 적합해도 명시적 terminal close capability가 없으면 주 에이전트 중심 순차 실행으로
둔다. 이미 ID가 발급된 예외 상황에서 close가 불가능하면 결과를 보존하고 unresolved cleanup을 운영상
위험으로 기록하며 완전한 실행으로 판정하지 않는다.
