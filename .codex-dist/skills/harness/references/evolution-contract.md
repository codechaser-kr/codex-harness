...
핵심은 자동 스크립트로 프로젝트를 바꾸는 데 있지 않다. 타겟 프로젝트에서 확인한 차이를 사람이 읽을 수 있는 로그, 역할 출력과 운영 감사 결과로 남기고, 사용자가 어느 로컬 하네스 자산이나 생성기 reference에 반영할지 판단할 수 있게 만드는 데 있다.
...
- 개선은 운영 감사 역할이 관찰과 후보 범위를 준비하고 사용자가 확정한 문서/스킬 변경으로 수행한다.
...
3. `latest-session-summary.md`에 후보 하네스 Phase와 학습 후보를 남긴다.
4. 운영 감사 역할이 관찰을 아래 분류표에 맞춰 가능한 승격 위치와 영향으로 연결한다.
5. 사용자가 반영 범위를 확정한 경우에만 `team-spec`, 역할 스킬, 운영 문서, QA 기준을 갱신한다.
...
| 생성기 반복 결함 | 여러 타겟 프로젝트에서 같은 종류의 계약 결함이 반복됨 | 사용자 확정 후 하네스 Phase 7 | 타겟 관찰 기록 | 전역 `SKILL.md`, 관련 reference |
...
- 승격 결정은 운영 감사 역할의 관찰·후보 준비와 사용자의 최종 결정으로 나눈다.
- 운영 감사 역할: 승격 위치 후보, 이유, 영향과 불확실성을 제시하며 최선의 선택을 판정하지 않는다.
- 사용자: 제안을 검토하고 최종 반영 여부와 범위를 확정한다.
- 흐름 조율 역할은 사용자 결정을 기록하고 확정된 범위만 하네스 Phase 7로 전달한다.
...
3. 운영 감사 역할이 학습 후보와 영향 자료를 제공하고 사용자가 반영 여부와 범위를 확정한다.
...
- `target-evaluation-playbook.md`: 타겟 관찰을 사용자 선택지, 로컬 보강과 생성기 환류 후보로 분리한다.
- `quality-evaluation-guide.md`: 사용자 판단을 위한 with-skill / without-skill 차이를 보존할 때 사용한다.
