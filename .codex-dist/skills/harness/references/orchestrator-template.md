...
- 위임 단위를 정할 때도 같은 수준의 판단을 해야 한다. 문서, 로그, handoff를 계속 유지해야 하는 핵심 역할은 팀 구조로 두고, 입력과 출력이 좁은 일회성 보조 분석만 따로 위임한다.
- fan-out 분석이 필요한 모노레포라도 최종 통합 책임은 항상 팀 역할로 다시 모으고, domain-analyst, harness-architect, qa-designer, validator 같은 중심 역할을 일회성 위임으로만 대체하지 않는다.
