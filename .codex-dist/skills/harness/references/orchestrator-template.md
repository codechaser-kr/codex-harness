...
- 지금 구조 설계를 시작해도 되는가
- 탐색 근거가 부족하다면 사용자에게 먼저 확인할 질문이 정리되었는가
...
## 6-1. Agent Teams / Subagents 선택 기준

orchestrator는 위임 단위를 정할 때도 같은 수준의 판단을 해야 한다.

- 문서, 로그, handoff를 계속 유지해야 하는 핵심 역할은 `Agent Teams`로 둔다.
- 입력과 출력이 좁고, 일회성 분석으로 끝나는 보조 작업만 `Subagents`로 위임한다.
- fan-out 분석이 필요한 모노레포라도, 최종 통합 책임은 항상 팀 역할로 다시 모은다.
- domain-analyst, harness-architect, qa-designer, validator 같은 중심 역할을 Subagents로만 대체하지 않는다.

---

## 6-2. 사용자 숙련도 / 작업 성격 반영

orchestrator는 요청을 받을 때 다음도 함께 본다.

- 사용자가 저장소 고유 용어와 파일 경로를 정확히 말하는가
- 요청 범위가 단일 보강인지, 구조 재설계인지, 운영 점검인지 분명한가
- 질문을 먼저 받아야 하는가, 아니면 바로 역할 시작이 가능한가

사용자 맥락이 약하면 질문 루프를 먼저 두고,  
맥락이 강하면 update 범위를 좁혀 바로 시작한다.
