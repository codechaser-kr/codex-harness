...
- `Phase 0 감사` -> `Phase 1 도메인/작업 분석` -> `Phase 2 팀 설계` -> `Phase 3 에이전트/스킬 생성` -> `Phase 4 QA/검증 구조` -> `Phase 5 역할별 최종 작성` -> `Phase 6 구조 검증` -> `Phase 7 품질 비교와 성숙도 평가`
...
- `Phase 0 감사` -> 필요한 `Phase` 재진입 -> `Phase 6 구조 검증` -> 필요 시 `Phase 7 품질 비교와 성숙도 평가`
