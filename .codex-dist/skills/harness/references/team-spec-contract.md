...
이 문서는 `team-spec.md`의 권한과 목적, 불변 조건, 생성 순서, 정본과 파생 산출물의 관계,
역할·agent·skill 간 책임을 정의한다.
...
- 이 문서는 Team Spec의 정책·권한·불변 조건·생성 순서·정본 관계를 소유한다.
- `team-spec-schema.md`는 필수 섹션·필드, 역할 인벤토리와 역할 카드 형식, 허용값·필드 매핑,
  작성 예시와 구조 검증 기준을 소유한다.
- 정책이나 생성 순서를 변경할 때는 이 문서를 먼저 수정하고, 구조·형식 표현이 영향을 받으면
  `team-spec-schema.md`를 함께 정렬한다.
- 필드·형식·구조 검증을 변경할 때는 `team-spec-schema.md`를 먼저 수정하고, 이 문서의 불변 조건이나
  생성 책임이 영향을 받을 때만 이 문서를 함께 정렬한다.
- 두 문서에 같은 설명이 필요하면 소유 문서만 규칙을 정의하고 다른 문서는 해당 규칙을 참조한다.
...
`team-spec.md`가 역할 팀의 권한·책임·생성 판단을 완결된 기준으로 제공해야 한다는 불변 조건은
이 문서가 소유한다. 필수 섹션 이름, 필드와 생략 가능 여부는 `team-spec-schema.md`의
`필수 섹션과 필드`를 따른다.

## 역할 스펙 초안 필수 필드

각 역할 초안이 목적·책임·입출력·실행 조건·재진입·완료 기준과 생성 메타데이터를 충분히 제공해야
한다는 불변 조건은 이 문서가 소유한다. 필수 필드의 정확한 목록과 역할 카드 작성 형식은
`team-spec-schema.md`의 `필수 섹션과 필드`와 `역할별 실행 기준`을 따른다.

## 최종 역할 인벤토리 계약

최종 역할 인벤토리는 역할 팀을 생성 자산으로 전달하는 단일 정본이며, agent·skill·config보다 먼저
확정돼야 한다. 파생 자산은 이 인벤토리의 역할 집합과 실행 설정을 바꾸거나 별도 정본으로 재정의할 수
없다.

인벤토리의 fenced 블록 형식, 컬럼, 허용값, 필드 매핑과 구조 검증 기준은
`team-spec-schema.md`의 `Codex 생성 정보`와 `검증 포인트`를 따른다.
...
- 코드 수정 요청을 별도 역할에 위임하는 시작 진입 역할의 우선 입력에는 `team-spec.md` 최종 역할 인벤토리와 역할 카드, `orchestration-plan.md`의 코드 수정 라우팅 규칙을 포함한다.
- 코드 수정 후보 역할은 자신의 역할 카드에 선언된 허용 변경 범위와 책임에서만 선택될 수 있다. 해당 선언이 없거나 요청과 맞지 않으면 후보가 아니다.
- 코드 수정 라우팅은 정확히 하나의 후보만 선택할 수 있다. 후보 0개, 복수 후보, team-spec/agent/skill 불일치 또는 누락이면 `run_harness`는 파일을 수정하지 않는 중단 결과를 남긴다.
- 선택 결과의 출력 규칙은 역할 식별자, agent config path, local skill path, agent TOML에서 확인한 실행 설정과 라우팅 근거를 포함해야 한다.
...
- 코드 수정 요청의 시작 진입 역할과 실제 코드 수정 역할이 분리되고, 실제 수정 역할의 `role_id`, `agent_file`, agent config, local skill, model/reasoning/sandbox가 상호 일치한다.
...
- `team-spec-schema.md`: 필수 섹션·필드, 역할 카드와 인벤토리 형식, 허용값·매핑·예시와 구조 검증을 정의한다.
