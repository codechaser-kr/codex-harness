...
- 예를 들어 거래/승인 흐름이 핵심이면 `transaction-flow-dev`, `approval-reviewer`, `flow-qa` 같은 이름이 더 적절할 수 있다.
- 런타임 경계 중심 프로젝트면 `runtime-boundary-dev`, `boundary-reviewer` 같은 이름이 더 적절할 수 있다.
...
- 같은 정책 도메인을 맡는 역할들이 같은 정책 원천을 공유하는가
...
- 생성된 `.codex/agents/*.toml`과 `.agents/skills/*/SKILL.md`가 역할별 시작 체크리스트, 판단 기준, 완료 기준, 다음 역할 기준을 중복 복제하지 않는가
