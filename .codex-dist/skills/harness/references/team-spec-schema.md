...
- 구현형 역할은 수정 가능한 경계, 먼저 확인할 계약, 구현 후 남겨야 할 검증 메모가 중요하다.
- 리뷰형 역할은 리스크 우선순위, finding 출력 형식, 승인/반려 기준이 중요하다.
- QA형 역할은 수동/자동 검증 분리, 실패 비용 우선순위, 미실행 위험 기록이 중요하다.
- 조율형 역할은 시작 역할 판단, handoff 연결, 재진입 기준, 세션 종료 조건이 중요하다.
