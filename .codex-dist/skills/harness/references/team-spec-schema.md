...
- 출력 규칙에는 용어 표기 기준을 포함한다. 하네스 생명주기 단계는 `하네스 Phase N`으로 쓰고, 제품/로드맵/개발 단계는 저장소 도메인에 맞는 수식어를 붙이며, 한국어만으로 의미가 모호한 기술 용어는 `감사(audit)`처럼 영어를 괄호 안에 병기한다.
...
- 특정 문서명은 고정하지 않고, 저장소 재독해 결과를 바탕으로 역할 판단에 필요한 원천 문서를 결정한다.
