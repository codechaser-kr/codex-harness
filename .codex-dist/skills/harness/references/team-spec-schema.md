...
현재 메타시스템에서는 seed 역할과 프로젝트 특화 역할의 생성 수준이 다를 수 있다.

- seed 역할: 이미 품질 기준이 높은 상세 SKILL 템플릿을 유지한다.
- 프로젝트 특화 역할: team-spec을 바탕으로 기본 SKILL을 생성하고, 해당 프로젝트 안에서 역할이 직접 구체화한다.

즉 목표는 모든 역할을 같은 수준의 빈 스텁으로 만드는 것이 아니라,
기준 계약이 필요한 seed는 강하게 유지하고,
프로젝트 고유 역할은 team-spec 중심으로 열어 두는 것이다.
