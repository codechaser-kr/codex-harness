...
- QA 역할은 변경 범위가 제품 정책, API/계약, 데이터 모델, 보안/감사/비용/외부 연동, 화면 사양, 로드맵 상태 중 어디에 닿는지 보고 필요한 설계 원천을 선택해 읽도록 정의한다.
- 변경 범위에 필요한 원천 문서가 없으면 QA 역할은 보류 판단, 미확인 위험, 다음 재진입 역할을 남겨야 한다.
- 프론트엔드 역할이 상태 기반 화면, 권한 기반 화면, 도메인 엔티티 목록/상세/수정 화면, enum/API 응답 필드 기반 UI, 필터/탭/뱃지/상태 라벨을 맡으면 데이터 모델 또는 API schema 원천을 우선 입력에 포함한다.
- 프론트엔드 데이터 원천은 저장소 재독해 결과를 바탕으로 데이터 모델 문서, API schema, OpenAPI/GraphQL schema, shared type package, backend DTO/response type, 앱별 `AGENTS.md`가 지정한 API 타입 원천 중에서 결정한다.
- 프론트엔드 역할은 자체 enum/status 문자열을 새로 만들지 않고 shared type이나 API response type을 우선 사용하도록 정의한다.
- 프론트엔드 화면 상태에 필요한 데이터 원천이 없으면 API/backend 역할 재진입 조건 또는 보류 판단을 역할 스펙에 남긴다.
...
- 프론트엔드 역할이 상태/권한/엔티티/API 필드 기반 UI를 맡을 때 데이터 모델 또는 API schema 원천을 우선 입력으로 갖는가
- 프론트엔드 역할이 shared type이나 API response type을 우선 사용하고 자체 enum/status 문자열을 만들지 않도록 정의됐는가
- 프론트엔드 데이터 원천이 없을 때 API/backend 재진입 조건 또는 보류 판단이 남아 있는가
