...
- 표현 계층 역할이 상태/권한/도메인 객체/계약 필드 기반 출력을 맡을 때 데이터 모델 또는 인터페이스 계약 원천을 우선 입력으로 갖는다.
- 표현 계층 역할이 저장소의 기존 타입/계약 원천을 우선 사용하고 구조화된 상태값이나 필드명을 임의로 만들지 않도록 정의돼 있다.
- 표현 계층 데이터 원천이 없을 때 생산자 역할 재진입 조건 또는 보류 판단이 명시돼 있다.
- 역할 목적에 외부 연동, 추적성, 비용/사용량, 보안, 민감정보, 모델/서비스 제공자 같은 정책 도메인이 포함될 때 관련 정책 문서가 우선 입력에 포함돼 있다.
