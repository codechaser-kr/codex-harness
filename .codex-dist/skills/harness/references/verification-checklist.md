...
- 역할의 목적, 책임, 주요 출력과 우선 입력 문서가 정합적이다.
- 선행 정책, 구현 순서, 라우팅 판단, 보류 질문을 맡는 역할이 필요한 정책/설계/로드맵 원천을 우선 입력으로 갖는다.
- 계획/조율 역할이 구현 역할보다 먼저 정책 판단을 해야 하는데 관련 원천 문서를 누락하지 않는다.
- 우선 입력에 포함된 문서는 해당 역할의 판단이나 출력 근거로 설명 가능하다.
- QA 역할이 변경 diff, 대상 앱 또는 패키지의 `AGENTS.md`, team-spec 설계 원천 목록, 변경 범위에 해당하는 설계/정책/계약 문서, 최신 재진입 상태를 우선 입력으로 갖는다.
- QA 역할이 고정 문서 몇 개만 읽도록 제한되지 않고 변경 범위에 맞는 설계 원천을 선택해 읽도록 정의돼 있다.
- QA 역할이 필요한 원천 문서가 없을 때 보류 판단, 미확인 위험, 다음 재진입 역할을 남기도록 정의돼 있다.
- 프론트엔드 역할이 상태/권한/엔티티/API 필드 기반 UI를 맡을 때 데이터 모델 또는 API schema 원천을 우선 입력으로 갖는다.
- 프론트엔드 역할이 shared type이나 API response type을 우선 사용하고 자체 enum/status 문자열을 만들지 않도록 정의돼 있다.
- 프론트엔드 데이터 원천이 없을 때 API/backend 역할 재진입 조건 또는 보류 판단이 명시돼 있다.
- 역할 목적에 MCP, 감사 로그, AI 사용량/쿼터, 보안, 민감정보, LLM Provider 같은 정책 도메인이 포함될 때 관련 정책 문서가 우선 입력에 포함돼 있다.
