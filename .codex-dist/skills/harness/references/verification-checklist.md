...
- 역할 목적에 MCP, 감사 로그, AI Credit, 보안, 민감정보, LLM Provider 같은 정책 도메인이 포함될 때 관련 정책 문서가 우선 입력에 포함돼 있다.
- 정책 문서가 존재하는데 해당 책임 역할의 우선 입력에서 누락되지 않았다.
- 같은 정책 도메인을 맡는 역할들이 서로 다른 입력 문서를 읽어 정책 drift를 만들지 않는다.
- 정책 문서가 없을 경우 보류 판단이나 대체 원천이 명시돼 있다.
...
- `latest-session-summary.md`의 현재 Phase 문구가 로드맵 완료 상태를 과장하지 않는다.
- 현재 Phase의 로드맵 항목 중 실제 산출물 확인이 필요한 항목이 완료 근거 없이 남아 있지 않다.
- 절차 문서만 있는 항목을 실제 산출물 완료로 잘못 판정하지 않는다.
- 다음 Phase 진입 문구가 Phase 잔여 작업을 건너뛰게 만들지 않는다.
