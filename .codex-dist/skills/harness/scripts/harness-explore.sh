...
역할 팀이 실제 구조와 도메인을 다시 읽을 때 참고할 후보 단서를 정리합니다.
해석 문서를 대신하지 않으며, 자동 수집된 시작점 후보와 관련 경로를 공유하는 것을 목적으로 합니다.

## 요약

- 현재 작업 디렉토리: \`$ROOT_DIR\`
- 하네스 운영 모드: \`$(detect_harness_operation_mode)\`
- 탐색은 대표 시작점 후보, 관련 코드 경로, 검증 자산, 실행/설정 경로, 저장소 용어 단서를 수집합니다.

## 대표 진입점

이 목록은 자동으로 포착한 시작점 후보입니다. 실제 사용자 흐름의 출발점은 코드와 문서를 함께 읽으며 다시 적습니다.
EOF
...
이 목록은 함께 읽을 관련 코드 경로 후보입니다. 경계 설명은 실제 책임과 소비 관계를 다시 읽으며 적습니다.
EOF
...
이 목록은 자동으로 포착한 검증 자산 후보입니다. 실제 핵심 흐름과 얼마나 연결되는지는 다시 읽으며 정리합니다.
EOF
...
이 목록은 실행, 빌드, 배포, 검증에 연결될 수 있는 설정 후보입니다. 실제 운영 경로와의 연결은 다시 읽으며 적습니다.
EOF
...
이 목록은 문서에서 포착한 용어 후보입니다. 실제 도메인 설명에 쓸 용어는 코드와 함께 다시 읽으며 고릅니다.
EOF
