...
성숙도 판단 뒤에는 다음 재진입 규칙도 함께 적는다.

- \`운영 가능\`: 현재 구조를 유지하고 필요 시 \`Phase 7\` 회고만 남긴다.
- \`재작성 필요\`: 가장 약한 축에 맞춰 \`Phase 1\`~\`Phase 5\` 중 하나를 시작점으로 다시 제안한다.
- \`재구성 필요\`: 부분 갱신보다 \`Phase 0\` 감사와 명시적 재구성을 먼저 제안한다.
...
- 현재 루프 판단을 함께 적는다.
- 현재 성숙도 판단을 함께 적는다.
- 성숙도 판단 다음 줄에는 반드시 \`다음 재진입 Phase\` 또는 \`유지 판단\`을 적는다.
...
- \`운영 가능\`이면 유지 판단과 함께 필요 시 \`Phase 7\` 회고만 남긴다.
- \`재작성 필요\`이면 가장 약한 축에 맞는 \`다음 재진입 Phase\`를 \`Phase 1\`~\`Phase 5\` 중 하나로 직접 적는다.
- \`재구성 필요\`이면 \`Phase 0\` 감사와 명시적 재구성을 먼저 적는다.
