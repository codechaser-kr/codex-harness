...
"---
name: domain-analyst
description: 저장소의 목적, 대표 진입점 후보, 관련 코드 경로, 실행·검증 경로, 하네스 관점의 핵심 관심사를 분석합니다. 프로젝트 구조 분석, 탐색 근거 정리, 핵심 흐름 해석, 하네스 출발점 정의가 필요할 때 적극적으로 사용합니다.
---
...
5. 실행·검증 경로와 하네스 관점에서 중요한 영역 메모를 남긴다.
...
- \`exploration-notes\`는 판단 결과가 아니라 후보 단서 문서로 보고, 실제 저장소 구조와 흐름을 다시 읽는 메모를 남긴다.
...
분석 결과를 바탕으로, 이 프로젝트에 어떤 역할 팀과 산출물 구조가 필요한지 적는다.

## 주요 작업

1. domain-analysis를 읽는다.
2. 필요한 역할 구성을 적는다.
3. 어떤 역할은 유지하고 어떤 역할은 줄일지 적는다.
4. 실행 하네스 구조 메모를 \`.harness/reports/harness-architecture.md\`에 남긴다.
5. 경계 목록을 그대로 반복하지 말고, 왜 이 역할 분리가 필요한지 구조 해석으로 번역한다.
...
1. 어떤 경계와 위험 때문에 역할 분리가 필요한가
2. 각 역할이 어느 해석 층을 맡는가
3. 어떤 역할은 유지하고 어떤 역할은 줄일 수 있는가
4. 어떤 산출물과 handoff가 실제로 필요하다고 보는가
...
- skill-scaffolder가 바로 반영할 수 있는 수준으로 구조를 명확히 적는다.
- 역할을 과도하게 늘리지 않는다.
- 역할 이름만 나열하지 말고, 각 역할이 왜 필요한지 저장소 경계와 위험을 기준으로 설명한다.
- 비슷한 역할 설명을 여러 섹션에서 반복하기보다, 한 번 근거를 제시한 뒤 유지·축소 기준으로 이어 적는다.
...
- \`역할 배치\`는 역할 목록이 아니라 저장소 경계와 위험을 어떤 해석 층으로 나눌지 설명하는 구조 문서가 되어야 한다.
...
"---
name: qa-designer
description: 프로젝트 실행 하네스에서 필요한 품질 기준, 질문 메모, 경계면 메모를 설계합니다. QA 전략 수립, 품질 기준 정리, 구조 정합성 읽기가 필요할 때 적극적으로 사용합니다.
---
...
이 프로젝트에서 무엇을 반복적으로 볼지 적고, 역할 팀이 공유할 QA 관점 메모를 남긴다.
...
5. 경계나 파일 목록을 그대로 옮기지 말고, 무엇을 왜 검증해야 하는지 품질 질문으로 번역한다.
...
1. 어떤 경계와 흐름이 실패 비용이 큰가
2. 무엇을 앞에서 검토할 것인가
3. 어떤 검증은 자동화하고 어떤 검증은 수동 확인해야 하는가
4. 위 해석을 뒷받침하는 대표 파일·경로는 무엇인가
...
- validator가 실제 피드백을 줄 수 있게 질문 메모를 또렷하게 적는다.
...
- \`우선 검토 질문\`과 \`변경 유형별 최소 체크\`는 같은 말을 반복하지 말고, 질문과 실행 기준을 구분해 적는다.
- \`.harness/*\` 문서는 특별한 요청이 없으면 한글로 작성한다. 파일명은 기존 영문 이름을 유지한다.
...
6. 순서 목록을 그대로 늘어놓지 말고, 왜 이 시작점과 재진입이 맞는지 운영 근거로 설명한다.
...
1. 어떤 요청 유형에서 어디서 시작해야 하는가
2. 왜 그 시작점이 맞는가
3. 어떤 경우에 앞 단계로 되돌아가야 하는가
4. 어떤 handoff가 실제로 필요한가
...
- \`시작 분기\`는 단순 순서표가 아니라, 어떤 요청에서 왜 그 역할부터 시작하는지 설명하는 규칙이어야 한다.
- \`순서 조정 및 재진입 기준\`은 앞뒤 단계 이름 반복보다, 어떤 신호가 나오면 어디로 되돌아갈지 먼저 적는다.
- \`.harness/*\` 문서는 특별한 요청이 없으면 한글로 작성한다. 파일명은 기존 영문 이름을 유지한다.
...
"---
name: run-harness
description: 프로젝트 로컬 실행 하네스 팀을 실제로 기동하는 진입점입니다. 현재 저장소 상태를 보고 어떤 역할 흐름을 앞에 열지 적고, 분석·설계·QA·오케스트레이션·검증 흐름을 시작하거나 다시 이을 때 적극적으로 사용합니다.
---
...
4. 탐색 근거가 부족하거나 빈 프로젝트에 가까우면 사용자 질문 메모를 남긴다.
5. domain-analysis가 비어 있거나 약하면 domain-analyst로 여는 흐름 메모를 남긴다.
6. 구조 설계나 패키지 경계 설명이 약하면 harness-architect를 앞에 두는 흐름 메모를 남긴다.
7. QA 기준이 약하면 qa-designer 재호출 메모를 남긴다.
8. 흐름 연결이 약하면 orchestrator 중심 재정렬 메모를 남긴다.
9. 마지막에 validator 관점의 최소 구조 메모를 남긴다.
...
- 시작 근거에는 현재 가장 약한 경계 또는 문서, 그리고 왜 이 시작 역할 메모가 열리는지를 함께 적는다.
...
- 요청: \"QA 문서만 다시 작성\" → 흐름: 기존 확장, 단일 보고서 갱신 → 시작: \`harness-update.sh --qa\` 검토 후 qa-designer
- 요청: \"domain-analysis만 오래됐음\" → 흐름: 기존 확장, 단일 보고서 갱신 → 시작: \`harness-update.sh --domain\` 검토 후 domain-analyst
