...
분석 결과를 바탕으로, 이 프로젝트에 어떤 역할 팀과 산출물 구조가 필요한지 최종 구조 문서로 작성한다.

## 주요 작업

1. domain-analysis를 읽는다.
2. init가 남긴 골격 문구를 유지하지 않고, 저장소 기준 구조 설명으로 다시 쓴다.
3. 필요한 역할 구성을 적는다.
4. 어떤 역할은 유지하고 어떤 역할은 줄일지 적는다.
5. 실행 하네스 구조를 \`.harness/reports/harness-architecture.md\`에 최종 결과로 작성한다.
...
이 프로젝트에서 무엇을 반복적으로 검증해야 하는지 정리하고, 역할 팀이 공유할 최종 QA 전략 문서를 작성한다.
