...
"---
name: domain-analyst
description: 저장소의 목적, 대표 진입점 후보, 관련 코드 경로, 실행·검증 경로를 분석하고 최종 도메인 분석 문서를 작성합니다. 프로젝트 구조 분석, 탐색 근거 해석, 핵심 흐름 정리가 필요할 때 적극적으로 사용합니다.
---
...
현재 저장소가 무엇을 하는 프로젝트인지 파악하고, 이후 운영 문서가 공통으로 참조할 수 있는 분석 결과를 만든다.
...
- 운영 문서 작성의 첫 단계
- harness-architecture, qa-strategy, orchestration-plan의 입력을 만든다
...
- domain-analyst의 결과를 단순 요약하지 말고, 저장소 운영 구조로 번역한다.
...
2. 저장소 기준 운영 계획을 최종 문서로 직접 작성한다.
...
현재 저장소 상태를 보고, 어떤 운영 문서부터 다시 써야 하는지와 어느 Phase부터 다시 시작해야 하는지 결정한다.

## 주요 작업

1. 현재 \`.harness/reports/*\`, \`.codex/skills/*\`, 로그 파일 상태를 읽는다.
2. 최종 운영 문서가 이미 있는지, 아직 작성되지 않았는지 먼저 가른다.
...
- init 직후라면 최종 운영 문서가 아직 없고, verify 전에 역할 작성이 필요하다는 점을 함께 적는다.
...
- 새 프로젝트에서 \`harness-init.sh\`만 끝난 상태라면 domain-analyst → harness-architect → qa-designer → orchestrator 순서로 운영 문서를 먼저 작성하고, 그 다음 validator와 \`harness-verify.sh\`를 둔다.
...
- 최종 운영 문서가 비어 있거나 generic 문장만 남아 있으면 \`harness-verify.sh\`보다 역할 재작성부터 앞에 둔다.
