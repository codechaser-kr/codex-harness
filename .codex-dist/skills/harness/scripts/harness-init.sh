...
- 저장소 고유 명사와 업무 용어를 일반론으로 바꾸지 않는다.
- 도메인 설명은 최소 3개의 실제 파일 또는 경로 근거와 연결한다.
- 패키지 책임, 상태 흐름, 배포 경계가 보이면 파일 근거와 함께 직접 설명한다.
...
- \`저장소 요약\`은 일반 기술 스택 설명보다 실제 사용자 문제와 핵심 도메인 흐름을 먼저 설명한다.
- \`저장소 고유 근거\`에는 실제로 도메인 설명을 뒷받침하는 대표 파일과 경로를 우선 남긴다.
...
- 후속 역할이 저장소를 직접 읽더라도 \`.claude\`, \`.codex\`, \`.agents\` 같은 AI 설정 디렉토리는 도메인 근거에서 제외한다.
- \`build\`, \`dist\`, \`coverage\` 같은 생성 산출물 경로는 실제 시작 흐름이나 소비 경계를 설명하는 근거로 사용하지 않는다.
