...
- \`역할 배치\`는 역할 목록이 아니라 저장소 경계와 위험을 어떤 판단 층으로 나눌지 설명하는 구조 문서가 되어야 한다.
...
"---
name: run-harness
description: 프로젝트 로컬 실행 하네스 팀을 실제로 기동하는 진입점입니다. 현재 저장소 상태를 보고 어떤 역할을 먼저 사용해야 할지 판단하고, 분석·설계·QA·오케스트레이션·검증 흐름을 시작하거나 다시 이어야 할 때 적극적으로 사용합니다.
---
...
- 판단 근거에는 현재 가장 약한 경계 또는 문서, 그리고 왜 이 시작 역할이 맞는지를 함께 적는다.
...
- 요청: \"QA 문서만 다시 작성\" → 판단: 기존 확장, 단일 보고서 갱신 → 시작: \`harness-update.sh --qa\` 검토 후 qa-designer
- 요청: \"domain-analysis만 오래됐음\" → 판단: 기존 확장, 단일 보고서 갱신 → 시작: \`harness-update.sh --domain\` 검토 후 domain-analyst
