...
프로젝트 로컬 실행 하네스가 최소한의 구조, 설명, 연결성을 갖추었는지 판독하고 입력 문서와 메타시스템 문서가 서로 다른 목적을 유지하는지 확인한다.
...
5. 저장소 입력 문서와 메타시스템 문서의 목적이 섞인 지점을 찾는다.
6. 저장소 맞춤 근거가 일반론으로 약해지거나 generic 문장으로 되돌아간 지점을 찾는다.
...
2. 어떤 문서가 입력 문서/메타시스템 문서 목적을 잃었는가
