...
4. 실행 하네스 구조를 \`.harness/reports/harness-architecture.md\`에 최종 결과로 작성한다.
...
이 프로젝트에서 무엇을 반복적으로 봐야 하는지 정리하고, 역할 팀이 공유할 QA 전략 문서를 작성한다.

## 주요 작업

1. domain-analysis와 harness-architecture를 함께 읽는다.
2. 품질 실패 유형을 적는다.
3. 반복해서 사용할 질문 세트를 적는다.
...
4. 프로젝트 로컬 실행 하네스 구조를 \`.harness/reports/orchestration-plan.md\`에 최종 결과로 작성한다.
...
현재 저장소 상태를 보고, 실행 하네스 팀이 어떤 순서로 움직일지 결정하고 시작한다.
...
4. 탐색 근거가 부족하거나 빈 프로젝트에 가까우면 사용자 질문을 남긴다.
5. domain-analysis가 비어 있거나 약하면 domain-analyst부터 시작한다.
6. 구조 설계나 패키지 경계 설명이 약하면 harness-architect를 앞에 둔다.
7. QA 기준이 약하면 qa-designer를 다시 호출한다.
8. 흐름 연결이 약하면 orchestrator 중심으로 재정렬한다.
9. 마지막에 validator 관점의 최소 구조 확인을 남긴다.
