...
1. 저장소의 목적과 범위를 추정한다.
2. 대표 진입점 후보와 함께 읽어야 할 관련 코드 경로를 확인한다.
...
- \`exploration-notes\`는 판단 결과가 아니라 후보 단서 문서로 보고, 실제 저장소 구조와 흐름을 다시 읽어 검증한다.
- \`핵심 실행 흐름\`은 경로 목록이 아니라 사용자 흐름, 상태 전파, 소비 관계를 설명하는 문장으로 정리한다.
...
- validator가 실제 피드백을 줄 수 있게 검토 질문을 명확히 적는다.
...
"---
name: run-harness
description: 프로젝트 로컬 실행 하네스 팀을 실제로 기동하는 진입점입니다. 현재 저장소 상태를 보고 어떤 역할 흐름을 먼저 열지 정리하고, 분석·설계·QA·오케스트레이션·검증 흐름을 시작하거나 다시 이어야 할 때 적극적으로 사용합니다.
---
...
- 시작 근거에는 현재 가장 약한 경계 또는 문서, 그리고 왜 이 시작 역할을 검토하는지를 함께 적는다.
