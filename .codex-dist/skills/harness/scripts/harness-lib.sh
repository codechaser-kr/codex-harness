...
EOF
      ...
      printf '%s\n' "- 하네스 문서는 위 단서를 바탕으로 역할 팀이 직접 작성합니다."
...
EOF
      ...
      printf '%s\n' "- 검증 비용이 큰 경계와 수동 확인이 필요한 결합 지점을 역할 관점으로 나눠 보는 것"
...
EOF
      ...
      printf '%s\n' "- 대표 진입점 파일과 영향도가 큰 변경 경계는 역할 팀이 함께 읽어 정리합니다."
...
- 저장소 사실 확인, 구조 설계, 스킬 반영, QA 설계, 흐름 조율, 최종 검증, 기동 진입점이 이 문서에서 함께 다뤄질 후보 층입니다.
- templates/scenarios 같은 반복 자산은 저장소에 반복 패턴이 축적될 때만 검토할 확장 자산 후보입니다.
- run-harness는 현재 상태 판단과 실제 작업 역할 호출이 갈라지는 지점을 설명할 때 함께 봐야 할 역할입니다.
...
- orchestrator: 작업 시작점, handoff, 재진입 루프를 하나의 운영 흐름으로 정리할 역할입니다.
- validator: 일반론 회귀와 되돌림 지점을 점검할 역할입니다.
- run-harness: 현재 요청과 문서 상태를 읽고 시작 역할과 다음 역할을 고를 역할입니다.

### 역할 유지와 조정 기준

- 역할 수는 고정 답안이 아니라 경계 종류, 검증 비용, 운영 복잡도를 함께 읽고 정합니다.
...
- 역할을 줄이거나 늘릴 때는 각 역할의 산출물과 판단 기준이 실제로 겹치는지부터 다시 확인합니다.
...
- 자동 재생성 결과라도 실제 탐색 근거를 반영한 분석이 먼저 옵니다.
- 프로젝트 특화 판단이 필요한 부분은 역할 팀이 직접 다시 적습니다.

## 다음 단계

- domain-analysis와 qa-strategy가 저장소 고유 명사와 실패 지점을 직접 적는지 다시 확인합니다.
...
범용 하네스 1차 단계에서는 아래 항목이 먼저 드러납니다.
...
- 저장소 전용 질문은 구조 일반론 대신 실제 파일, 테스트 유틸, 진입점 단서를 바탕으로 직접 적습니다.
...
- 매 요청마다 전체 질문 세트를 다 확인하지 않고, 이번 변경이 걸리는 축을 먼저 적고 나머지는 위험 순위를 매깁니다.
...
- 저장소 고유 확인 단서를 기준으로 QA 질문을 로컬 고유 명사와 실패 지점까지 직접 적습니다.
...
6. validator가 현재 구성이 최소 요건을 충족하는지 확인한다.
...
- 시작 역할과 재진입 지점은 사용자 요청 유형, 경계 영향 범위, 남은 검증 비용을 함께 읽으며 정리합니다.
...
4. qa-designer가 이번 축에 맞는 검토 질문과 체크포인트를 직접 적습니다.
5. orchestrator가 작업 시작 루프와 검증 루프를 운영 메모로 정리합니다.
6. validator가 산출물이 일반론으로 흐르지 않았는지 마지막으로 확인합니다.
...
- 구조 또는 문서 정비: run-harness -> skill-scaffolder -> orchestrator -> validator
  - 역할 설명과 템플릿 반영이 중심일 때 함께 검토할 수 있는 흐름 후보입니다.
...
- qa-designer -> orchestrator: 검토 질문과 최소 체크를 적은 뒤 어떤 루프로 운영할지 넘깁니다.
...
- 반복적으로 등장하는 우회 흐름이 있으면 team-playbook과 함께 루프를 다시 정리합니다.
...
- 이 문서는 현재 저장소의 관련 코드 경로 후보를 어떤 역할 팀이 나눠서 읽을지 정리합니다.
...
- qa-designer: 품질 질문과 검토 기준을 적는 역할입니다.
- orchestrator: 역할 간 handoff와 재진입 흐름을 정리하는 역할입니다.
- validator: 회귀와 누락을 점검하는 역할입니다.
- run-harness: 시작 역할 결정과 사용자 확인 질문을 여는 역할입니다.

### 실전 분류 예시

- 변경 중심은 사용자 진입점, 공용 계층, 별도 실행 환경 중 어디에 가까운지로 갈립니다.
- 단일 수정처럼 보여도 소비 경로나 공용 계층까지 번지면 domain-analyst와 qa-designer를 앞에 두는 흐름을 검토합니다.
- 문서 정비 요청이라도 handoff나 다음 진입점이 바뀌면 orchestrator와 validator를 함께 붙이는 흐름을 검토합니다.

### 운영 기준

- 역할 수는 많을수록 좋은 것이 아니라, 현재 저장소에서 실제로 구분이 필요한 책임 경계만큼만 둡니다.
- 역할을 추가하거나 줄일 때는 각 역할의 산출물과 판단 기준이 실제로 어떻게 갈리는지 다시 확인합니다.
...
- architecture와 orchestration은 이 팀 구조 설명과 같은 경계를 함께 가리키는지 다시 확인합니다.
EOF
      ;;
  esac
}
...
- 먼저 읽을 문서는 보통 domain-analysis, orchestration-plan, qa-strategy 중 현재 요청과 직접 연결되는 문서입니다.
