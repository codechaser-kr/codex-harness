...
- 저장소 고유 확인 단서를 기준으로 QA 질문은 로컬 고유 명사와 실패 지점 메모로 이어집니다.
...
4. qa-designer가 이번 축에 맞는 검토 질문과 체크포인트 메모를 남깁니다.
...
- run-harness: 시작 흐름 메모와 사용자 확인 질문을 여는 역할입니다.
...
2. run-harness는 현재 상태를 보고, 탐색 근거가 부족하면 사용자 확인 질문으로 이어지고, 근거가 충분하면 필요한 역할 순서 메모가 열립니다.
