...
### Agent Teams / Subagents 판단 기준

- 문서, 로그, handoff를 계속 유지해야 하는 중심 역할은 Agent Teams로 둡니다.
- 입력과 출력이 좁고 일회성 분석으로 닫히는 보조 작업만 Subagents로 둡니다.
- fan-out 분석이 필요해도 최종 통합 책임은 항상 역할 팀으로 다시 모읍니다.
- domain-analyst, harness-architect, qa-designer, validator 같은 중심 역할은 장기 운영 구조로 유지합니다.
...
- 문서와 로그에 남아야 하는 중심 역할은 Agent Teams로 두고, run-harness가 그 순서를 조율합니다.
- 입력과 출력이 좁고 단발성 보조 판단이면 Subagents를 고려할 수 있습니다.
- fan-out 분석을 하더라도 최종 handoff와 재진입 기준은 역할 팀 문서로 다시 통합합니다.

### 사용자 숙련도 / 작업 성격 판단 기준

- 저장소 고유 용어, 파일 경로, 영향 범위를 정확히 말하면 시작 역할을 더 직접적으로 고릅니다.
- 요청이 추상적이거나 맥락이 약하면 사용자 확인 질문과 탐색 보강부터 둡니다.
- 단일 문서 보강, 구조 재설계, 운영 점검 중 무엇인지 먼저 나누고 그에 맞는 루프를 고릅니다.
