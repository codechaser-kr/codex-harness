...
- 구조 설명이 흐리면 harness-architect, 체크리스트가 약하면 qa-designer, handoff가 끊기면 orchestrator를 우선 재진입시킵니다.
...
- 먼저 읽을 문서와 나중에 볼 문서를 구분해 빠르게 시작하고, 역할 호출이나 전환 이유가 바뀌면 그 근거도 함께 남깁니다.
