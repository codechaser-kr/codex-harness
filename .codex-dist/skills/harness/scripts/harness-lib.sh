...
1. run-harness가 요청이 기능 변경인지, 구조 정리인지, 검증 보강인지 먼저 분류합니다.
2. 변경이 $key_axes_hint 중 어느 축에 걸리는지 판단합니다.
3. 영향 범위가 넓으면 domain-analyst와 qa-designer를 먼저 붙이고, 좁으면 orchestrator가 바로 작업 흐름을 정리합니다.

## 권장 흐름

1. domain-analyst가 실제 코드 경로와 변경 경계를 먼저 재확인합니다.
