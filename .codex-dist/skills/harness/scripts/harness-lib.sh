...
- 문서, 로그, handoff를 계속 유지해야 하는 중심 역할은 Agent Teams로 두고, 입력과 출력이 좁은 일회성 보조 판단만 Subagents로 둡니다.
- fan-out 분석이 필요해도 최종 통합 책임은 항상 역할 팀으로 다시 모으고, domain-analyst, harness-architect, qa-designer, validator 같은 중심 역할은 장기 운영 구조로 유지합니다.
...
- 문서와 로그에 남아야 하는 중심 역할은 Agent Teams로 두고 run-harness가 그 순서를 조율하며, 입력과 출력이 좁은 단발성 보조 판단만 Subagents로 고려합니다.
- 저장소 고유 용어, 파일 경로, 영향 범위를 정확히 말하면 시작 역할을 더 직접적으로 고르고, 요청이 추상적이거나 맥락이 약하면 사용자 확인 질문과 탐색 보강부터 둡니다.
- 단일 문서 보강인지, 구조 재설계인지, 운영 점검인지 먼저 나눈 뒤 그에 맞는 루프를 고릅니다.
