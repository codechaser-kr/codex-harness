...
if [ "$EXPLORATION_CONTEXT_LEVEL" = "충분" ]; then
  ...
  if [ -f ".harness/reports/harness-architecture.md" ]; then
    ...
    check_contains_any_hint ".harness/reports/harness-architecture.md" "Agent Teams|Subagents|Subagent|일회성 보조 판단|장기 운영 구조" "아키텍처 실행 단위 기준"
  ...
  if [ -f ".harness/reports/orchestration-plan.md" ]; then
    ...
    check_contains_any_hint ".harness/reports/orchestration-plan.md" "Agent Teams|Subagents|Subagent|일회성 보조 판단|장기 운영 구조" "오케스트레이션 실행 단위 기준"
