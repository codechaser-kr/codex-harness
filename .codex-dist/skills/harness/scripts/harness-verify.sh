...
if [ -f ".codex/skills/run-harness/SKILL.md" ]; then
  ...
  check_contains_any_hint ".codex/skills/run-harness/SKILL.md" "Agent Teams|Subagents|Subagent" "실행 단위 판단 규칙"
  check_contains_any_hint ".codex/skills/run-harness/SKILL.md" "파이프라인|생성-검증|팬아웃|오케스트레이션 중심" "아키텍처 패턴 판단 규칙"
  check_contains_any_hint ".codex/skills/run-harness/SKILL.md" "사용자 숙련도|작업 성격|저장소 고유 용어|추상적" "사용자 맥락 판단 규칙"
...
if [ "$EXPLORATION_CONTEXT_LEVEL" = "충분" ]; then
  ...
  if [ -f ".harness/reports/harness-architecture.md" ]; then
    ...
    check_contains_any_hint ".harness/reports/harness-architecture.md" "Agent Teams|Subagents|Subagent" "아키텍처 실행 단위 기준"
  ...
  if [ -f ".harness/reports/orchestration-plan.md" ]; then
    ...
    check_contains_any_hint ".harness/reports/orchestration-plan.md" "Agent Teams|Subagents|Subagent" "오케스트레이션 실행 단위 기준"
    check_contains_any_hint ".harness/reports/orchestration-plan.md" "사용자 숙련도|작업 성격|저장소 고유 용어|추상적" "오케스트레이션 사용자 맥락 기준"
