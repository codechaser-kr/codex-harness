...
`claude/code-review`는 `$cc:review` companion review가 아니라 `claude -p`에서 `/code-review <base-branch>...<head-branch>` command를 호출해 코드 리뷰를 요청하는 모드입니다. `/code-review --comment`와 `/code-review --fix`는 사용하지 않으며, GitHub comment 게시와 파일 수정은 Workflow Engine 후속 단계에서만 수행합니다.
