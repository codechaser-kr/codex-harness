...
그러면 전역 `harness` 스킬이 현재 저장소와 사용자 입력을 함께 읽고, 프로젝트 내부에 로컬 실행 하네스 스킬과 루트 기준 AI 탐색 구조를 생성합니다.
...
`하네스 Phase 2 프로젝트 맞춤 에이전트 팀 설계`에서 현재 저장소 도메인에 맞는 `team-spec`을 만들고, `하네스 Phase 3 에이전트 정의 생성`에서 그 스펙을 바탕으로 프로젝트 특화 에이전트 팀을 동적으로 생성합니다.
...
0. 현재 하네스 현황 감사(audit)를 먼저 수행합니다.
...
신규 구축에서는 생성 직후의 하네스도 자기진화 루프를 시작할 수 있어야 합니다. 초기 생성물은 다음 시작 역할, 다음 하네스 재진입 Phase, 학습 후보 기록 위치를 남겨야 하며, 이 기준은 `.codex-dist/skills/harness/references/initial-generation-contract.md`에서 관리합니다.
