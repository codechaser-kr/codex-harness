...
이 저장소는 문서만 만드는 도구가 아닙니다. 저장소를 탐색해 근거를 수집하고, 그 결과를 공통 입력으로 삼아 역할 기반 스킬 팀, 오케스트레이션 흐름, QA/검증 구조, `run-harness` 진입점, 운영 루프를 설계합니다.
...
탐색 근거가 아직 부족한 경우에는 바로 역할을 확정하지 않고, `run-harness`가 먼저 프로젝트 유형, 핵심 사용자, 첫 성공 시나리오 같은 확인 질문을 제시한 뒤 다음 역할 흐름으로 넘어가도록 설계되어 있습니다.
...
- 실제 판단은 대표 진입점, 핵심 모듈, 테스트 자산, 설정/배포 경로, 저장소 고유 용어를 읽고 내립니다.
...
- `초기`: 대표 진입점, 코드 경계, 테스트 자산 같은 근거가 거의 수집되지 않은 상태입니다. 이 단계에서는 역할을 단정하지 않고 사용자 질문과 첫 성공 흐름 정리가 우선입니다.
- `제한적`: 일부 근거는 수집됐지만 경계나 검증 자산이 충분히 연결되지 않은 상태입니다. 이 단계에서는 문서를 보강하되, 저장소 특화 판단은 더 좁게 하고 추가 탐색을 병행합니다.
- `충분`: 대표 진입점, 주요 코드 경계, 테스트/실행 경로, 저장소 고유 용어가 연결된 상태입니다. 이 단계에서 역할 팀, 운영 규칙, 선택 자산, verify 기준을 더 구체적으로 적용합니다.
