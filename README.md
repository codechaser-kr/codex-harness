...
또는 `이 저장소에 맞는 역할 팀과 QA 흐름을 설계해줘`, `프로젝트 특화 에이전트 팀을 구성해줘`처럼 요청해도 됩니다.

그러면 전역 `harness` 스킬이 현재 저장소를 읽고, 프로젝트 내부에 로컬 실행 하네스 스킬과 루트 기준 AI 탐색 구조를 생성합니다. 초기화 직후에는 `exploration-notes.md`, `project-setup.md`, `team-spec.md` 같은 시작 문서만 준비되고, 이후 역할 팀이 저장소를 다시 읽으며 분석 문서, 운영 문서, 로컬 역할 자산을 순서대로 채웁니다.
