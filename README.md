...
`harness`는 Codex용 전역 스킬로, 현재 프로젝트에 맞는 **로컬 실행 하네스 팀**을 생성합니다.

이 저장소는 문서만 만드는 도구가 아닙니다. 프로젝트 안에 역할 기반 스킬 팀, 오케스트레이션 흐름, QA/검증 구조, `run-harness` 진입점을 갖춘 실행 기반을 만드는 데 초점이 있습니다.
...
빈 프로젝트이거나 저장소 단서가 약한 경우에는 바로 역할을 확정하지 않고, `run-harness`가 먼저 사용자에게 프로젝트 유형, 핵심 사용자, 첫 성공 시나리오 같은 확인 질문을 제시한 뒤 다음 역할 흐름으로 넘어가도록 설계되어 있습니다.
...
저장소 단서가 충분하지 않으면, 위 흐름에 들어가기 전에 `run-harness`가 먼저 짧은 사용자 확인 질문을 만들고 그 답을 `domain-analysis`와 이후 오케스트레이션의 입력으로 사용합니다.

즉, 단순 문서 생성이 아니라 `분석 -> 역할 설계 -> 스킬 생성 -> 오케스트레이션 -> QA/검증 -> 로그 운영` 흐름을 갖춘 실행 하네스 기반을 만드는 것이 목적입니다.
...
이 저장소의 현재 목표는 **범용 프로젝트 실행 하네스 팀 생성기**입니다.

즉, 이 저장소는 어떤 프로젝트에나 공통으로 적용할 수 있는 역할 팀과 운영 기반을 먼저 만드는 데 집중합니다. 반대로 프로젝트마다 정의가 크게 달라지는 실행 기준과 검증 절차는 기본값으로 고정하지 않습니다.
