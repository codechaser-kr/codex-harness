...
assert_contains "$(cat "$TMP_ROOT/empty-project/.codex/config.toml")" "# 초기 seed 에이전트 팀 설정" "빈 프로젝트 config seed 안내"
assert_dir "$TMP_ROOT/empty-project/.codex/agents"
assert_file "$TMP_ROOT/empty-project/.codex/agents/run-harness.toml"
assert_contains "$(cat "$TMP_ROOT/empty-project/.codex/agents/run-harness.toml")" "# 초기 seed 정의" "빈 프로젝트 run-harness agent seed 안내"
...
assert_contains "$(cat "$HARNESS_REF_DIR/../SKILL.md")" "초기 seed 역할 팀" "전역 harness skill seed 역할 팀 표현"
